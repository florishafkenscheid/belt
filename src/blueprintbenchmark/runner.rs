...
impl BlueprintBenchmarkRunner {
    ...
    pub async fn run_all(
        ...
        for bp_file in &blueprint_files {
            let new_save_path = self.factorio.run_for_blueprint(
        ...
        return BenchmarkRunner::new(BenchmarkConfig { 
            saves_dir: self.config.blueprints_dir.clone(), // doesn't matter, not really used...
             ...
             headless: self.config.headless
        }, FactorioExecutor::new(self.factorio.executable_path().to_path_buf())).run_all(converted_save_paths, running).await;
    }
}
