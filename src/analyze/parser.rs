...
use crate::{benchmark::{parser::{BenchmarkResult, BenchmarkRun}, runner::VerboseData}, core::error::Result};

/// Read results.csv and reconstruct BenchmarkResult vector
pub fn read_benchmark_results(csv_path: &PathBuf) -> Result<Vec<BenchmarkResult>> {
    ...
    for result in reader.records() {
        ...
        let result = results_map.entry(save_name.clone()).or_insert_with(|| BenchmarkResult {
            save_name,
            ticks,
            runs: Vec::new(),
            factorio_version,
...
pub fn read_verbose_data(verbose_csv_files: &[PathBuf]) -> Result<HashMap<String, Vec<VerboseData>>> {
